...
class Model {
  ...
  static buildPermissions(klass) {
    ...
    if (klass.permissions.select) {
      metadata.select_permissions = [
        {
          role: "user",
          permission: { filter: klass.permissions.select },
        },
      ];
    }

    if (klass.permissions.insert) {
      metadata.insert_permissions = [
        {
          role: "user",
          permission: { check: klass.permissions.insert },
        },
      ];
    }

    if (klass.permissions.update || klass.permissions.postUpdate) {
      ...
      if (klass.permissions.update)
        metadata.update_permissions[0].permission.filter =
          klass.permissions.update;

      if (klass.permissions.postUpdate)
        metadata.update_permissions[0].permission.check =
          klass.permissions.postUpdate;
    }

    if (klass.permissions.delete) {
      metadata.delete_permissions = [
        {
          role: "user",
          permission: { filter: klass.permissions.delete },
        },
      ];
    }
